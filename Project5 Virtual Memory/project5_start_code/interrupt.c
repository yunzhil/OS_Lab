...
#define TIMER_INTERVAL 300000000
    ...
    if((signed)get_cp0_index() >= 0) {
        ...
        *(p_pte + entrylo_offset) = pte;
        ...
        if(old_pte & 0x80000000) {
            ...
            v_begin = current_running->entry_point & 0xffffe000;
            loc_begin = current_running->loc - (current_running->entry_point - v_begin);
            offset = ((Badvpn2 << 13) | (entrylo_offset << 12)) - v_begin;
            if(offset < current_running->size + (current_running->entry_point - v_begin))
                bcopy(loc_begin + offset, page_vaddr(free_index), PAGE_SIZE);
        }
       // print_str(23 + tlb_refill_count + tlb_invalid_count, 1, " ");
        printf(19,1,"TLB Invalid: Badvaddr is %x, old_pte is %x, pte is %x,pid is %d",  Badvaddr, old_pte, pte,EntryHi & 0xff);
    } else {
        tlb_refill_count++;
        ...
        asm volatile("tlbwr");
       // print_str(23 + tlb_refill_count + tlb_invalid_count, 1, " ");
        printf(20,1,"TLB Refill : Badvaddr is %x, EntryLo0 is %x, EntryLo1 is %x,pid is %d",  Badvaddr, *(p_pte), *(p_pte + 1),EntryHi & 0xff);
